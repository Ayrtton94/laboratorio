...
$('#warningusaurio').hide();
$('#consultarsunat').hide();
$('#agregarusuarioorden').hide();

$('#dniorden').bind("change paste keyup", function() {
  if($('#dniorden').val()=="12312312"){
      
$('#warningusaurio').show();
$('#agregarusuarioorden').show();
$('#consultarsunat').show();
 ...
 */
  ...
  if($('#ordersChart').length) {
    var options2 = {
      ...
      xaxis: {
        type: 'text',
        categories: ["1ER MUESTREO", "2DO MUESTREO", "3ER MUESTREO",],
      },
    };
...
if($('#ventaspormes').length) {
    var options2 = {
      ...
      series: [{
        name: '',
        data: [36, 77, 52, 60, 43, 12, 100, 200, 180]
      }],
      xaxis: {
        type: 'text',
    ...
    new ApexCharts(document.querySelector("#ventaspormes"),options2).render();
  }
 ...
 $.validator.setDefaults({
  ...
  $(function() {
    ...
    $("#signupForm_area_empresa").validate({
      rules: {
        nombre: {
          required: true,
          minlength: 3
      ...
      messages: {
        nombre: {
          required: "Ingrese nombre de un área",
          minlength: "Ingrese 3 caracteres como mínimo"
        },
...
;(function($){
 ...
 if($('#dashboardDate').length) {
  ...
  if($('#dateorden').length) {
    var date = new Date();
    var today = new Date(date.getFullYear(), date.getMonth(), date.getDate());
    $('#dateorden').datepicker({
      language: 'es',
      format: "mm/dd/yyyy",
      todayHighlight: true,
      autoclose: true
    });
    $('#dateorden').datepicker('setDate', today);
  }
  ...
  'use strict';
    if (type === 'basic') {
      ...
      swalWithBootstrapButtons.fire({
        title: '¿Desea Eliminar de Base de datos?',
        ...
        if (result.value) {
          swalWithBootstrapButtons.fire(
            'Eliminado!',
            'Se ha quitado de la base de datos.',
            'success'
          )
        } else if (
